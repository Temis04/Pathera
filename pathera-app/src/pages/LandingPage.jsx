...
const LandingPage = () => {
  ...
  const handleProPlanClick = async () => {
    ...
    if (!STRIPE.PRICE_ID_MONTHLY) {
      alert('Stripe is not configured yet. Please add PRICE_ID_MONTHLY to constants.js');
      return;
    }

    setIsLoadingCheckout(true);
    try {
      const { url, error } = await createCheckoutSession(user.id, STRIPE.PRICE_ID_MONTHLY);
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {pricingPlans.map((plan, index) => (
              ...
              >
                ...
                <button
                  onClick={() => {
                    if (plan.name === 'Pro') {
                      ...
                      window.location.href = 'mailto:contact@pathera.com?subject=Enterprise Plan Inquiry';
                    } else {
                      // Free plan - just open auth modal
                      setIsAuthModalOpen(true);
                    }
                  }}
                  disabled={isLoadingCheckout && plan.name === 'Pro'}
                ...
                >
                  {isLoadingCheckout && plan.name === 'Pro' 
                    ? 'Loading...' 
